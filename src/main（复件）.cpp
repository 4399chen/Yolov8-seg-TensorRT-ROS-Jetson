...
void Callback(const sensor_msgs::ImageConstPtr &msg, YOLOv8_seg* yolov8, ros::NodeHandle& nh, image_transport::ImageTransport& it){

    //make a ROS publisher topic "/boundingboxes" as "result_pub"
    static ros::Publisher result_pub = nh.advertise<fsd_common_msgs::YoloConeDetections>("/yolov8_seg/boundingboxes", 1);
    // image_transport::Publisher pub = it.advertise("/yolov8_seg/mask", 1);
...
int main(int argc, char** argv)
{
    ...
    if (isROSTopic) {
        ...
        image_transport::Subscriber sub = it.subscribe(input_image_topic_name, 1, boost::bind(&Callback, _1, yolov8, nh, it) );
        ...
        if (isVideo) {
            ...
            while (cap.read(image)) {
                ...
                yolov8->draw_mask(image, res_mask, objs, CLASS_NAMES, COLORS, MASK_COLORS);
